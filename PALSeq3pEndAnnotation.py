...
def generateFastq(palBedFile):
	...
	for line in palBedFile:
		...
		if line.split("\t")[5] == "-": pos = int(line.split("\t")[6]) # downstream mapping coordinate is the left one for the negative strand because they're already revcomp in PAL-seq
		elif line.split("\t")[5] == "+": pos = int(line.split("\t")[7])
		else: 
	...
	for accessionHash, posDict in dictTags.iteritems():
		...
		while len(posDict.keys()) != 0: #this while loop is set up to remove keys from the dictionary as it evaluates the position dict. 
			...
			for pos in posDict.keys(): #could probably do without the two for loops with a sort method
				if greatestTagPos - 10 <= pos <=  greatestTagPos + 10: #check if another tag is within 10
...
dictTags = generateFastq(palBedFile)
